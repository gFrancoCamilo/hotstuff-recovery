    ...
    with open(filename, 'r') as f:
        ...
        match stat:
            ...
            case "consensus-latency":
                match_string = re.search(r"Consensus latency:\s(.*) ms", file_content)
                return match_string.group(1).replace(",","")
            case "e2e-tps":
...
def plot_stats(mean_original, err_original, mean_our, err_our):
    ...
    x = [i for i in range(10000,60000,10000)]
    ...
    plt.tight_layout()
    plt.grid()
    plt.savefig('consensus-latency-rate.pdf', dpi=600)
